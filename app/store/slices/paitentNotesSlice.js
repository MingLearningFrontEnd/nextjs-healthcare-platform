...
export const fetchPatientNotesData = createAsyncThunk(
  'patientNotesSlice/fetchPatientNotesData',
  async (patientId, { rejectWithValue }) => {
    try {
      const response = await fetch(`${API_BASE_URL}//patientnotes/${patientId}`,
      {
        method: 'GET',
        headers: {
...
const patientNotesSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchPatientNotesData.pending, (state) => {
        state.loading = true;
        state.error = null;
      })
      .addCase(fetchPatientNotesData.fulfilled, (state, { payload }) => {
        state.loading = false;
        state.patient = payload;
      })
      .addCase(fetchPatientNotesData.rejected, (state, action) => {
        state.loading = false;
...
export const { clearPatient } = patientNotesSlice.actions;
