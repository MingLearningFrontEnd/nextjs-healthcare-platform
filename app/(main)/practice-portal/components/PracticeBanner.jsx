...
import { fetchPracticeData } from '@/app/store/slices/practiceSlice'

export default function PracticeBanner() {
  const dispatch = useDispatch();
  const { practice, loading, error } = useSelector((state) => state.practiceSlice);
  ...
  if (error) {
    return <div className="py-20 text-center text-3xl">Error: {error}</div>;
  }

  if (!practice) {
    return <div className="py-20 text-center text-3xl">No practice data available</div>;
  }

  const { name, email, id, role } = practice;
    ...
    <Card className="rounded-sm w-full bg-white border-0 border-b-2 border-black relative">
      <CardContent className="relative flex flex-col sm:flex-row items-center sm:items-start justify-between p-4 sm:p-5 pt-8 sm:pt-6 gap-4">
        ...
        <div className="w-full sm:w-auto mt-2 sm:mt-0 sm:ml-auto sm:mb-3 flex justify-center sm:justify-end">
          <PracticeNavigation practiceId={id} />
        </div>
      </CardContent>
    </Card>
  );
